...
def main():
    ...
    for chunk in df_iter:
        for idx, row in chunk.iterrows():
            ...
            try:
                # Based on test_csv, the extracted_json doesn't wrap content in a "content" key
                # It's flat: title, tables, metadata, sections, document_type, entities (maybe in sections?)
    ...
    for row, ext in selected_pages:
        ...
        # fallback: search in sections if entities not at top level
        if not ents and 'sections' in ext:
            for s in ext['sections']:
                if isinstance(s, dict) and 'entities' in s:
                    ents.extend(s['entities'])
                    
        entities_count += len(ents)
        
        for e in ents:
            if e.get('confidence') in ['high', 'medium', 'low'] or 'value' in e: # allow all confidences for now
                val = e.get('value', '')
                if not val: continue
    ...
    type_desig = type_designations.most_common(1)[0][0] if type_designations else "AW139" # Hardcode AW139 based on cwd if still None
    operator = operators.most_common(1)[0][0] if operators else None
    
    if has_helicopter_signs or type_desig == "AW139":
        asset_class = "rotorcraft"
        subtype = "HELICOPTER"
        
    dates.sort()
    dossier_date = dates[-1] if dates else "2024-01-01" # dummy date if none found
