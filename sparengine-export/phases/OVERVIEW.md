...
1. `graph_export.json` — lossless graph projection (panel HTML reads it).
...
4. `asset_graph.html`  — copy of `asset_graph_template_panels.html` with `{{ASSET_TITLE}}` substituted. Renders Critical Items, Mandatory Checklist, Audit Quality, and Lease-return banner. **Do not generate HTML from scratch.**
5. `progress.log`      — per-phase verification counts. Every phase appends to this.

The graph proper is queried via the Neo4j Browser at `http://localhost:7474`.
...
Layer 1: TIER GROUPS       — KILLED. ATA chapter is the regulatory grouping; tiers
                             are derived at viz time via tier_views.cypher.
...
| ~~3~~ | — | **DELETED** — Tier killed (Q6). The ATA→tier mapping in `tiers_and_ata.md` is now consumed at viz time, not as a graph layer. |
...
| 10 | Coding    | `phase10_export.md`         | `graph_export.json`, `restore.cypher`, `tier_views.cypher` (the per-asset deliverables); also auto-applies `captions.cypher` so Browser auto-captions are meaningful |
| viz| Coding    | `phase_viz.md`              | `asset_graph.html` (panel-only template + JSON) |

Reference files (load when the phase says to):

- `csv_and_ocr.md` — CSV schema and the structure of `extracted_json`. **Read once before Phase 1.**
- `document_types.md` — closed enum of document_type strings.
- `tiers_and_ata.md` — ATA→tier mapping, used at viz time for groupings + `tier_views.cypher`.
- `schema.cypher` — exact constraint + index + fulltext-index definitions.
- `captions.cypher` — caption patch (sets `name` on every node so Browser displays meaningfully).
...
- `finding_types.md` — the closed list of finding-type strings (use exact spelling).
...
| **10** Export | Coding | Database → JSON. Pure Cypher reads + APOC export. |
| **viz** | Coding | Template substitution. |
...
2. Confirm `asset_graph_template_panels.html` exists at `sparengine-export/asset_graph_template_panels.html`. **Never generate it from scratch.**
3. Confirm Neo4j is reachable: `verify_schema(driver)` returns OK.
4. Run `phase0_orientation.md`. Read its MANDATORY VERIFICATION before declaring Phase 0 done.
5. Run Phase 1. Verify `count(:Page) ≈ csv_row_count` and `verify_phase_1` reports 0 fact-orphans.
6. Continue phase by phase. After each phase, **load only the next phase's file** — don't re-read the entire OVERVIEW or unrelated phases.
7. At every phase end, append the verification counts to `progress.log`.
8. After Phase 10, the per-asset deliverables (`graph_export.json` + `restore.cypher` + `tier_views.cypher`) are in the workdir.
9. After viz, **open `asset_graph.html` in a browser** and confirm it shows your data, not the sample data shipped in the template.
