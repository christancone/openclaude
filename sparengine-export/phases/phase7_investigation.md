...
**Intent.** Walk every component's events chronologically; raise `:Finding` nodes against the **Investigation Discipline checklist** (`investigation_discipline.md`) and the **Severity Matrix** (`severity_matrix.md`). Every finding must trace to a page via `:EVIDENCED_BY` (the DAL enforces this).

**Reference files (load all):**
- `investigation_discipline.md` — the 9-step checklist for any "missing" finding
- `severity_matrix.md` — Level 1 / 2 / 3 mapping per component class
- `finding_types.md` — closed list of category strings
...
| Node | Properties |
|---|---|
| `:Finding` | `severity` ∈ {`level_1`, `level_2`, `level_3`}, `category` (closed enum from finding_types.md), `title`, `description` (≥80 chars, must cite page), `recommended_action`, `status="OPEN"` |
...
For any finding category that involves "missing X" (FORM1_MISSING, AD_COMPLIANCE_UNVERIFIED, etc.), you MUST run all 9 disciplines from `investigation_discipline.md` before raising the finding:
...
9. **oem_typical** — known OEM filing patterns (from data_quality_rules.md)
...
### 4. Common categories (from `finding_types.md`)
