...
- `references/tiers_and_ata.md` (asset detection signals — section "Asset detection signals (Phase 2 confirmation)")
- `references/finding_types.md` (for `CONTEXT_DISCREPANCY`)
...
Apply fuzzy-match for whitespace and Ltd/Limited variants on `operator`. Apply CAMP-as-concept-not-software rule from `references/data_quality_rules.md` on `mis_system`.
