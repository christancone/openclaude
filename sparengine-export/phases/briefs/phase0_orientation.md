...
- `references/csv_and_ocr.md` (CSV schema + extracted_json structure)
- `references/tiers_and_ata.md` (asset detection signals)
...
2. **Aggregate `entities[]` and `metadata`** across those pages:
   ...
   - Detect `asset_class` and `subtype` per `references/tiers_and_ata.md` rules.

3. **Build `expected_tiers`** from the detected `asset_class` (per `references/tiers_and_ata.md`). Engine-only → `["ENGINE"]`. Aircraft → `["ENGINE","LANDING_GEAR","PROPELLER","AIRFRAME","AVIONICS"]` (+ `APU` if helicopter or detected; + `ROTOR_SYSTEM`, `TRANSMISSION` if helicopter).
