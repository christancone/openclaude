...
For each `coverage[item] in {"missing", "stale"}`, raise a `:Finding` keyed `f"finding::asset::{item.upper()}"` with the corresponding category from `references/finding_types.md`. Use a representative evidence page (e.g. the first page of the asset_profile.json reference, or the asset's first :Page).
