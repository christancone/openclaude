...
Also auto-applies `phases/cypher/captions.cypher` so Browser auto-captions on every node label are meaningful.

**Reference files:**
- `references/tiers_and_ata.md` (the ATA→tier mapping for `tier_views.cypher`)
- `references/severity_matrix.md`
- `cypher/captions.cypher`
...
if shared_path.exists():
    ...
    # Schema is owned by phases/cypher/schema.cypher; replay shouldn't recreate it.
```
...
After all writes, run `phases/cypher/captions.cypher` to ensure every node has a `name` property (so Browser auto-captions display meaningfully). The DAL way:

```python
# Locate captions.cypher in the sparengine-export tree
for parent in [Path(__file__).resolve().parent, *Path(__file__).resolve().parents]:
    cand = parent / "sparengine-export" / "phases" / "cypher" / "captions.cypher"
    if cand.is_file():
        captions_file = cand
        break
