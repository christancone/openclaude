...
> **(v2-demo)** Tightening note: only promote a PN to a `:Component` when it
> appears on at least one page with `evidentiary_weight ∈ {primary, secondary}`.
> PNs that only appear on `reference` (manual) or `administrative` (invoice)
> pages stay as `:PartNumber` mentions, not `:Component`s. This is the change
> the analyser should compare across versions.
...
**Reference files:**
- `references/csv_and_ocr.md` (entities, tables)
- `references/tiers_and_ata.md` (informational — ATA → tier mapping; Tier nodes are no longer written, but the mapping drives threshold selection in rule 4)
- `references/data_quality_rules.md` (blocklist, OCR_SUSPECTED rules)
